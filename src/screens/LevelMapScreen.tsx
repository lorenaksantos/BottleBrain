...
const STATE_STYLE: Record<LevelState, { bg: string; border: string; text: string; label: string }> = {
...
function LevelCircle({ level, state, onPress }: {
  level: number; state: LevelState; onPress: (level: number, state: LevelState) => void;
}) {
  ...
  const handlePress = () => {
    if (state === 'locked') {
      tx.value = withSequence(
        withTiming(-7, { duration: 40 }), withTiming(7, { duration: 40 }),
  ...
  return (
    <Pressable onPress={handlePress} style={{ alignItems: 'center' }}>
      <Animated.View style={[{
        ...
        backgroundColor: style.bg, borderWidth: 2.5, borderColor: style.border,
        alignItems: 'center', justifyContent: 'center',
        shadowColor: '#2b1a66', shadowOpacity: state === 'locked' ? 0 : 0.18,
        shadowRadius: 8, shadowOffset: { width: 0, height: 3 },
        elevation: state === 'locked' ? 0 : 3,
      }, animStyle]}>
        {style.label ? (
          <>
            <Text style={{ fontSize: 18, color: style.text, fontWeight: '900' }}>{style.label}</Text>
            <Text style={{ fontSize: 10, color: style.text, fontWeight: '700', marginTop: 1, opacity: 0.85 }}>{level}</Text>
          </>
        ) : (
          <Text style={{ fontSize: 15, color: style.text, fontWeight: '900' }}>{level}</Text>
        )}
      </Animated.View>
    </Pressable>
  );
...
export function LevelMapScreen({ navigation }: Props) {
  ...
  const handleLevelPress = React.useCallback((level: number, state: LevelState) => {
    if (state === 'locked') {
      if (messageTimeout.current) clearTimeout(messageTimeout.current);
      setLockedMessage(t('complete_previous', { level: level - 1 }));
  ...
  const rows: number[][] = [];
  for (let i = 0; i < levels.length; i += COLUMNS) rows.push(levels.slice(i, i + COLUMNS));
    ...
    <View style={{ flex: 1, backgroundColor: '#F6F0FF' }}>
      ...
      {lockedMessage && (
        <View style={{
          ...
          backgroundColor: '#2b1a66', borderRadius: 14,
      ...
      {/* Legend */}
      <View style={{ flexDirection: 'row', gap: 14, paddingHorizontal: 20, paddingTop: 14, paddingBottom: 8, flexWrap: 'wrap' }}>
        {(['solved', 'skipped', 'unlocked', 'locked'] as LevelState[]).map(s => (
          <View key={s} style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
            ...
            <Text style={{ fontSize: 11, color: '#4a3b7a', fontWeight: '600' }}>
              {t(s)}
            </Text>
          </View>
        ))}
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40, gap: 16 }} showsVerticalScrollIndicator={false}>
        {rows.map((row, rowIdx) => (
          <View key={rowIdx} style={{ flexDirection: 'row', justifyContent: 'flex-start', gap: 14 }}>
            {row.map(level => (
              <LevelCircle
                key={level}
                level={level}
                state={isLoaded ? getLevelState(level, solvedLevels, skippedLevels, unlockedLevels) : 'locked'}
